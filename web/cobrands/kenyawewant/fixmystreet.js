...
// reCAPTCHA: no-op until page_controller sets real impl (script may call before page_controller runs)
fixmystreet.renderReportRecaptcha = function() {};
...
$.extend(fixmystreet.set_up, {
    ...
    $("form.validate").each(function(){
      fixmystreet.validator = $(this).validate({
        ...
        submitHandler: function(form) {
            if ($(form).data('report-form-submitting')) return;
            $(form).data('report-form-submitting', true);
  ...
  page_controller: function() {
    // reCAPTCHA: only render when user step is shown (called by script onload or on page_change)
    fixmystreet.renderReportRecaptcha = function() {
      if ($('.js-reporting-page--active').data('pageName') !== 'user') return;
      var $container = $('#report-recaptcha-container');
      if (!$container.length || $container.data('recaptcha-rendered')) return;
      var sitekey = $container.data('sitekey');
      if (!sitekey || typeof grecaptcha === 'undefined' || !grecaptcha.render) return;
      var doRender = function() {
        if ($container.data('recaptcha-rendered')) return;
        try {
          grecaptcha.render('report-recaptcha-container', { sitekey: sitekey });
          $container.data('recaptcha-rendered', true);
        } catch (err) {}
      };
      // Brief delay so the user step is visible before reCAPTCHA renders (avoids invisible widget)
    ...
    // Render reCAPTCHA when user reaches "user" step (script loads from template with nonce)
    $(fixmystreet).on('report_new:page_change', function(e, $from, $to) {
        if ($to.data('pageName') === 'user' && typeof fixmystreet.renderReportRecaptcha === 'function') {
            fixmystreet.renderReportRecaptcha();
        }
    });
  },
