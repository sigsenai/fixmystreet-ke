...
$(fixmystreet).on('display:report', function() {
    ...
    fixmystreet.staff_set_up.fancybox_moderation();  // bind Redact for AJAX-loaded report content //OSCAR
...
(function() {
    ...
    function mouse_pos(elt, e) {
        ...
        // Fixed: Use clientX/clientY (viewport coordinates) instead of pageX/pageY (document coordinates)
        // to match getBoundingClientRect() which returns viewport-relative coordinates
        // Also use r.left/r.top for better browser compatibility
        return { 'x': e.clientX - (r.left || r.x), 'y': e.clientY - (r.top || r.y) };
    }
